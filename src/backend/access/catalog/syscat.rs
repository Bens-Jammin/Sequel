...
pub fn init_syscat(table_name: &str, column_data: &Vec<(String, (ColumnType, bool))> ) {


    let file_path = format!( "{}/syscat.txt", table_dir(table_name) );
    let file = File::create( file_path ).unwrap();

    let indexes: Vec<IndexMetaData> = collect_index_data( table_name ).expect("failed to load data about table indices.");
...
fn collect_index_data(table_name: &str) -> io::Result<Vec<IndexMetaData>> {
    let mut index_data = Vec::new();

    // Replace "index_dir" with the actual directory path
    let path = index_dir( table_name );
...
pub fn update_syscat_on_disk(system_catalog: &SystemCatalog) {
    ...
    let file_path = format!( "{}/syscat.txt", table_dir(&system_catalog.table_name) );
    let file = File::create( file_path ).unwrap();
...
pub fn read_syscat(table_name: &str) -> Result<SystemCatalog, String> { 
    let file_path = format!( "{}/syscat.txt", table_dir(table_name) );
